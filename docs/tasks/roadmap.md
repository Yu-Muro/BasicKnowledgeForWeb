...
1. **進行中の Phase 5（メール認証）を完遂**: 既に PR #167 が進行中。OTP/メール検証は公開時点から厳密な試行回数制限と専用 rate limit を適用し、下記 Phase 6 で共通の多層防御へ拡張する。
...
- **Phase 5 → Phase 6**: Phase 5 の OTP ログイン / メール検証 verify エンドポイントにはDBの厳密な5回制限と専用 rate limit を先に適用する。Phase 6 ではこれを維持し、既存認証 API と Turnstile / WAF を含む共通の多層防御へ拡張する。
